...
/**
 * Request permission (if needed), get Expo push token, POST /auth/me/push-token as `app: driver`.
 */
export async function registerExpoPushWithApi({ api, authToken }) {
  ...
  if (!projectId || typeof projectId !== "string") {
    console.warn("[tnc driver push] Set EXPO_PUBLIC_EAS_PROJECT_ID after `eas init` for this app.");
  ...
  try {
    ...
    console.warn("[tnc driver push] getExpoPushTokenAsync", e?.message || e);
